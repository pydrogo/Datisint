...
{
    public class TaMasBaMaController : Controller
    {
        [Authorize(Roles = "TaMasBaMa")]
        public ActionResult Read()
        {
            using (DatisEntities de = new DatisEntities())
            {
                
                var all = de.SiteContents.Where(m => m.ArticleType == (int)DatisEnums.ArticleType.TamasBaMa).FirstOrDefault();
            ...
            else
            {
                using (DatisEntities de = new DatisEntities())
                {
                    if (siteContentModel.Id > 0)
                    {
                        ...
                        try
                        {
                            siteContentModel.ArticleType = (int)DatisEnums.ArticleType.TamasBaMa;
