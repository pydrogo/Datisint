...
namespace Datis.Controllers
{
    public class ApplicationController : Controller
    {
        public ActionResult Index()
        {
            ...
            {
                var res = de.SiteContents.Where(m => m.ArticleType == (int)DatisEnums.ArticleType.Application).Select(x => new ArticleVM()
                {
                    Id = x.Id,
                    Title = x.Title,
                    Details = x.Article,
                    Image = de.Images.FirstOrDefault(n => n.TableId == (int)DatisEnums.TableId.Application && n.ForeignId == x.Id).Path,
                    File = de.Files.FirstOrDefault(n => n.TableId == (int)DatisEnums.TableId.Application && n.ForeignId == x.Id).Path,
                    HasFile = de.Files.Any(n => n.TableId == (int)DatisEnums.TableId.Application && n.ForeignId == x.Id),
                    FileTypeLogo = de.Files.FirstOrDefault(n => n.TableId == (int)DatisEnums.TableId.Application && n.ForeignId == x.Id).Extension == ".zip" ? "Images/zip-icon.png"
                    : de.Files.FirstOrDefault(n => n.TableId == (int)DatisEnums.TableId.Application && n.ForeignId == x.Id).Extension == ".pdf" ? "Images/pdf-icon.png" : "Images/file-icon.png"
                }).ToList();
        ...
        public ActionResult Details(int Id)
        {
            using (DatisEntities de = new DatisEntities())
            {
                SiteContent sc = de.SiteContents.Where(m => m.ArticleType == (int)DatisEnums.ArticleType.Application).FirstOrDefault();
                var res = new ArticleDetailasVM()
                {
                    Id = sc.Id,
                    Title = sc.Title,
                    Details = sc.Article,
                    Files = de.Files.Where(m => m.ForeignId == sc.Id && m.TableId == (int)DatisEnums.TableId.Application)
        ...
        public ActionResult AjaxRead(TermsOfGarrantySearchVM searchOptions, StoreRequestParams parameters)
        {
            using (DatisEntities pe = new DatisEntities())
            {
                var all = pe.SiteContents.Where(x => (x.Title.Contains(searchOptions.Name) || searchOptions.Name == null)
                    ...
                    && x.ArticleType == (int)DatisEnums.ArticleType.Application).Select(m => new SiteContentVM()
                {
                    Id = m.Id,
                    Title = m.Title,
                    Article = m.Article
            ...
            else
            {
                using (DatisEntities de = new DatisEntities())
                {
                    if (siteContentModel.Id > 0)
                    {
                        ...
                        try
                        {
                            siteContentModel.ArticleType = (int)DatisEnums.ArticleType.Application;
