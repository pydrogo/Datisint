...
{
    [Authorize(Roles = "ReturnProductAdmin")]
    public class ReturnProductAdminController : Controller
    {
        public ActionResult Read()
        {
            using (DatisEntities de = new DatisEntities())
            {
                ...
                var all = de.SiteContents.Where(m => m.ArticleType == (int)DatisEnums.ArticleType.ReturnProduct).FirstOrDefault();
            ...
            else
            {
                using (DatisEntities de = new DatisEntities())
                {
                    if (siteContentModel.Id > 0)
                    {
                        ...
                        try
                        {
                            siteContentModel.ArticleType = (int)DatisEnums.ArticleType.ReturnProduct;
